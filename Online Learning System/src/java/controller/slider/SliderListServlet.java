...
public class SliderListServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Slider> list = dao.filterSliders(keyword, status);

        request.setAttribute("sliders", list);
        request.setAttribute("search", keyword);
        request.setAttribute("status", status);
        
        // Lưu keyword và status vào session để SliderDetailServlet có thể truy cập
        session.setAttribute("lastSearchKeyword", keyword); 
        session.setAttribute("lastSearchStatus", status);
